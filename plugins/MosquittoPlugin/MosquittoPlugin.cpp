...
void MosquittoPlugin::init(mc_control::MCGlobalController & gc, const mc_rtc::Configuration & config)
{
  ...
  persist_dir_ = config("persist_dir", std::string{fs::temp_directory_path() / "mosquitto_persist"});
  ...
  ctl.gui()->addElement(
      this, {"MosquittoPlugin"}, mc_rtc::gui::Input("Address", address_), mc_rtc::gui::Input("Port", port_),
      mc_rtc::gui::Input("Topic", topic_), mc_rtc::gui::Input("Client id", client_id_),
      mc_rtc::gui::Button("(Re)connect", [this]() { connect(); }), mc_rtc::gui::Label("Connected", connected_));
...
mc_control::GlobalPlugin::GlobalPluginConfiguration MosquittoPlugin::configuration()
{
