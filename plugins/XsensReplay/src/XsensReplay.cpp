...
std::string log_entry(const StrT &entry, const std::string &robot,
                      bool is_main) {
  if (is_main) {
    return entry;
  }
...
GetT get(const mc_rtc::log::FlatLog &log, const std::string &entry,
         const std::string &robot, bool is_main, size_t idx,
         const GetT &def = {}) {
  return log.get<GetT>(log_entry(entry, robot, is_main), idx, def);
}

/** Get the a robot's state from the log */
void log_to_robot(const mc_rtc::log::FlatLog &log, mc_rbdyn::Robot &robot,
                  bool is_main, size_t idx) {
  if (robot.mb().nrDof() == 0) {
    return;
  }
...
void update_datastore_fn(const mc_rtc::log::FlatLog &log,
                         const std::string &log_entry, size_t idx,
                         mc_rtc::DataStore &ds, const std::string &ds_entry) {
  ds.assign(ds_entry, *log.getRaw<CppT>(log_entry, idx));
}

template <typename CppT>
void init_datastore(const mc_rtc::log::FlatLog &log,
                    const std::string &log_entry, mc_rtc::DataStore &ds,
                    const std::string &ds_entry) {
  ds.make<CppT>(ds_entry, *log.getRaw<CppT>(log_entry, 0));
}

XsensReplay::update_datastore_fn_t
make_update_datastore_fn(const mc_rtc::log::FlatLog &log,
                         const std::string &log_entry, mc_rtc::DataStore &ds,
                         const std::string &ds_entry) {
...
#undef HANDLE_CASE
  default:
    mc_rtc::log::error_and_throw("Cannot convert {} to C++ type automatically",
                                 LogTypeName(type));
  }
}
...
void XsensReplay::init(mc_control::MCGlobalController &gc,
  ...
  ds.make_call("Replay::SkipIter", [this](size_t skipIter) {
    skipIter_ = std::max(static_cast<size_t>(1), skipIter);
  });
  ds.make_call("Replay::SetStartTime", [this, &gc](double startTime) {
    start_iter_ = timeToIter(startTime, gc.timestep());
  });
  ds.make_call("Replay::SetEndTime", [this, &gc](double endTime) {
    end_iter_ = timeToIter(endTime, gc.timestep());
  });
  ds.make_call("Replay::GetStartTime", [this, &gc]() {
    return static_cast<double>(start_iter_) * gc.timestep();
  });
  ds.make_call("Replay::GetCurrentTime", [this, &gc]() {
    return static_cast<double>(iters_) * gc.timestep();
  });
  ds.make_call("Replay::GetEndTime", [this, &gc]() {
    return (static_cast<double>(end_iter_) - 1.0) * gc.timestep();
  });
  ds.make_call("Replay::SetLog", [this, &ds, &gc](const std::string &logPath) {
    init_log(logPath, ds);
    reset(gc);
  });
  ...
  if (ds.has("Replay::Log")) {
    ...
    if (!config.has("log")) {
      mc_rtc::log::warning(
          "[XsensReplay] No log specified in the plugin configuration and no "
          "log available in the datastore at XsensReplay::Log");
    } else {
  ...
  if (pause_ && with_inputs_ && !with_outputs_) {
    mc_rtc::log::warning(
        "[XsensReplay] Cannot start paused if only inputs are replayed");
    pause_ = false;
  }
  std::string with_datastore_config =
      config("with-datastore-config", std::string(""));
  if (!with_datastore_config.empty()) {
    mc_rtc::log::info(
        "[XsensReplay] Loading log to datastore configuration from {}",
        with_datastore_config);
    log_to_datastore_ = mc_rtc::Configuration(with_datastore_config)
                            .
...
void XsensReplay::init_log(const std::string &logPath, mc_rtc::DataStore &ds) {
  ...
  if (preload_logs_.count(logPath) > 0) {
    mc_rtc::log::info("[XsensReplay] Loading log {} from preload cache",
                      logPath);
  ...
  if (start_iter_ > log_->size() - 1) {
    mc_rtc::log::error_and_throw(
        "[XsensReplay] Start time cannot be outside of the log");
  } else if (end_iter_ != 0 && end_iter_ > log_->size() - 1) {
    mc_rtc::log::warning(
        "[XsensReplay] End time cannot be greater than end time");
...
void XsensReplay::reset(mc_control::MCGlobalController &gc) {
  ...
  if (gc.controller().name_ != ctl_name_) {
    mc_rtc::log::warning("[XsensReplay] Reset with a different controller than "
                         "the initial one, jumping to the end of the log");
    iters_ = log_->size() - 1;
  }
  if (with_outputs_) {
    ...
    for (const auto &r : *robots_) {
      ...
      gc.controller().gui()->addElement(
          {"Robots"},
          mc_rtc::gui::Robot(r.name(),
                             [&r]() -> const mc_rbdyn::Robot & { return r; }));
    }
  }
  ...
  for (auto it = log_to_datastore_.begin(); it != log_to_datastore_.end();) {
    ...
    if (!log_->has(log_entry)) {
      mc_rtc::log::error(
          "[XsensReplay] Requested to map {} to {} but {} is not in the log",
          log_entry, ds_entry, log_entry);
      it = log_to_datastore_.erase(it);
      continue;
    }
    datastore_updates_.push_back(
        {log_entry, ds_entry,
         make_update_datastore_fn(*log_, log_entry, gc.controller().datastore(),
                                  ds_entry)});
    ++it;
  }
  ...
  gc.controller().gui()->addElement(
      {"XsensReplay"}, mc_rtc::gui::Button("Pause/Play", [this]() {
        if (with_inputs_ && !with_outputs_) {
          mc_rtc::log::warning("[XsensReplay] XsensReplay cannot be paused "
                               "when only inputs are replayed");
          return;
        }
        pause_ = !pause_;
      }));
  gc.controller().gui()->addElement(
      ...
      mc_rtc::gui::NumberSlider(
          "XsensReplay time",
          [this, &gc]() { return static_cast<double>(iters_) * gc.timestep(); },
          [this, &gc](double t) {
            if (with_inputs_ && !with_outputs_) {
              mc_rtc::log::warning("[XsensReplay] XsensReplay time cannot be "
                                   "set when only inputs are replayed");
              return;
            }
            size_t iter = static_cast<size_t>(std::floor(t / gc.timestep()));
            iters_ =
                std::max<size_t>(std::min<size_t>(iter, log_->size() - 1), 0);
          },
          0.0, static_cast<double>(log_->size()) * gc.timestep()),
      mc_rtc::gui::Label("/", [this, &gc]() {
        return static_cast<double>(log_->size()) * gc.timestep();
      }));
  ...
  if (with_inputs_ && log_->meta()) {
    ...
    for (const auto &[r, fs_calibs] : calibs) {
      ...
      for (const auto &[fs_name, calib] : fs_calibs) {
        auto &fs =
            const_cast<mc_rbdyn::ForceSensor &>(robot.forceSensor(fs_name));
        fs.loadCalibrator(
            mc_rbdyn::detail::ForceSensorCalibData::fromConfiguration(calib));
      }
    }
  }
...
void XsensReplay::before(mc_control::MCGlobalController &gc) {
  ...
  if (with_inputs_) {
    for (const auto &r : gc.controller().robots()) {
      ...
      if (r.refJointOrder().size()) {
        gc.setEncoderValues(r.name(),
                            get(log, "qIn", r.name(), is_main, iters_));
        gc.setEncoderVelocities(r.name(),
                                get(log, "alphaIn", r.name(), is_main, iters_));
        gc.setJointTorques(r.name(),
                           get(log, "tauIn", r.name(), is_main, iters_));
      }
      // Restore force sensor readings
      std::map<std::string, sva::ForceVecd> wrenches;
      for (const auto &fs : r.forceSensors()) {
        wrenches[fs.name()] = get(log, fs.name(), r.name(), is_main, iters_,
                                  sva::ForceVecd::Zero());
      }
      ...
      for (const auto &bs : r.bodySensors()) {
        poses[bs.name()] = get(log, bs.name() + "_position", r.name(), is_main,
                               iters_, def_vec);
        oris[bs.name()] = get(log, bs.name() + "_orientation", r.name(),
                              is_main, iters_, def_quat);
        linearVels[bs.name()] = get(log, bs.name() + "_linearVelocity",
                                    r.name(), is_main, iters_, def_vec);
        angularVels[bs.name()] = get(log, bs.name() + "_angularVelocity",
                                     r.name(), is_main, iters_, def_vec);
        linearAccels[bs.name()] = get(log, bs.name() + "_linearAcceleration",
                                      r.name(), is_main, iters_, def_vec);
        angularAccels[bs.name()] = get(log, bs.name() + "_angularAcceleration",
                                       r.name(), is_main, iters_, def_vec);
      }
      ...
      for (const auto &js : r.jointSensors()) {
        motorTemps[js.joint()] =
            get(log, "JointSensor_" + js.joint() + "_motorTemperature",
                r.name(), is_main, iters_, 0.0);
        driverTemps[js.joint()] =
            get(log, "JointSensor_" + js.joint() + "_driverTemperature",
                r.name(), is_main, iters_, 0.0);
        motorCurrents[js.joint()] =
            get(log, "JointSensor_" + js.joint() + "_motorCurrent", r.name(),
                is_main, iters_, 0.0);
      }
  ...
  for (auto &update_ds : datastore_updates_) {
    update_ds.update(*log_, update_ds.log_entry, iters_,
                     gc.controller().datastore(), update_ds.ds_entry);
  }
}
...
void XsensReplay::after(mc_control::MCGlobalController &gc) {
  ...
  if (with_outputs_) {
    for (auto &r : *robots_) {
      log_to_robot(*log_, r, r.name() == gc.controller().robot().name(),
                   iters_);
