...
void XsensReplay::init(mc_control::MCGlobalController &gc, const mc_rtc::Configuration &config)
{
  ...
  ds.make_call("Replay::SetPause", [this](bool pause)
               { pause_ = pause; });
  ds.make_call("Replay::IsFinished", [this]()
               { return finished_; });
  ...
  if (with_outputs_)
  {
    ...
    for (const auto &r : *robots_)
    {
      ...
      gc.controller().gui()->addElement({"Robots"},
                                        mc_rtc::gui::Robot(r.name(), [&r]() -> const mc_rbdyn::Robot &
                                                           { return r; }));
    }
  }
