...
struct XsensReplay : public mc_control::GlobalPlugin {
  ...
  /** The XsensReplay plugin configuration supports the same options as those in
   * Ticker::Configuration::XsensReplay
   *
   * The exceptions are:
   * - stop_after_log and exit_after_log are not supported
   * - if XsensReplay::Log is set in the datastore before this plugin is started
   * then it is assumed the log was loaded previously
   */
  ...
  using update_datastore_fn_t = void (*)(const mc_rtc::log::FlatLog &log,
                                         ...
                                         size_t idx, mc_rtc::DataStore &ds,
