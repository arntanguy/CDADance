...
void AutonomousInteraction::start(mc_control::fsm::Controller &ctl_)
{
  ...
  ctl_.gui()->addElement(this, {},
                         mc_rtc::gui::Button("FINISH INTERACTION",
                                             [this]()
                                             { finished_ = true; }));
...
bool AutonomousInteraction::run(mc_control::fsm::Controller &ctl)
{
  ...
  auto &receivedMsgObj = ctl.datastore().get<rosSubscriberData>(
      "rosSubscriber_msg");  // assign receivedMsgObj to false if cannot found
                             // "rosSubscriber_msg" in the datastore
  ...
  while (!finished_ && !find_msg)
  {
    if (subscriber_msg != "")
    {
      ...
      segIdx = stoi(subscriber_msg.substr(subscriber_msg.find("world") + 6,
                                          capture_charatersN));
      find_msg = true;
    }
    else
  ...
  if (find_msg)
  {
    debug_log("------ Return Index = {}; output = {} ------", segIdx,
              SeqName[segIdx]);
