...
void AutonomousInteraction::start(mc_control::fsm::Controller & ctl_)
{
  ...
  ctl_.gui()->addElement(this, {}, mc_rtc::gui::Button("FINISH INTERACTION", [this]() { finished_ = true; }));
...
bool AutonomousInteraction::run(mc_control::fsm::Controller & ctl)
{
  ...
  auto & receivedMsgObj =
      ctl.datastore().get<rosSubscriberData>("rosSubscriber_msg"); // assign receivedMsgObj to false if cannot found
